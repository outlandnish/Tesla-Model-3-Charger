...
static void ChargerStateMachine()
{
  ...
  uint8_t pcsChargeStatus = Param::GetInt(Param::CHG_STAT);
  ...
  case OFF:
    ...
    if (Param::GetBool(Param::Drive_En))
      state = DRIVE;
    break;
  case WAITSTART:
    if (CheckDelay())
  ...
  case ACTIVATE:
    ...
    break;
  case EVSEACTIVATE:
    if (pcsChargeStatus == 3) // Activate EVSE when PCS enters wait for AC state.
    {
